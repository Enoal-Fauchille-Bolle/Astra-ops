...
| **Appflowy**            | `/opt/k3s-data/appflowy/`                  | unchanged                                      | 52M             | 1    | dump only      | PostgreSQL                                                                                |
...
| **Scrutiny**            | `/opt/k3s-data/scrutiny/`                  | unchanged                                      | 408K            | 1    | dump only      | SQLite                                                                                    |
...
| **Jellyfin**            | `/opt/k3s-data/jellyfin/`                  | unchanged                                      | 65M             | 1    | files (no DB)  | SQLite (`jellyfin.db` + `library.db`) — reconstructible on scan, Tier 1 PBS is sufficient |
| `/etc/pve/`             | Astra host                                 | unchanged                                      | ~5M             | 1    | ✅ files       | —                                                                                         |
| `/etc/proxmox-backup/`  | Astra host                                 | unchanged                                      | ~5M             | 1    | ✅ files       | —                                                                                         |
| **Immich photos**       | `/mnt/data/k3s-pvc/immich/`                | `/opt/k3s-data/immich/library/` _(planned)_    | 6.4G            | 2    | ✅ files       | —                                                                                         |
| **Filebrowser files**   | `/mnt/data/k3s-pvc/filebrowser/`           | unchanged                                      | 9.1G            | 2    | ✅ files       | —                                                                                         |
| **Homer config**        | `/mnt/data/k3s-pvc/homer/`                 | `/opt/k3s-data/homer/` _(planned)_             | 5.3M            | 2    | ✅ files       | —                                                                                         |
| **Criteri-fresque**     | `/mnt/data/k3s-pvc/criteri-fresque/`       | `/opt/k3s-data/criteri-fresque/` _(planned)_   | 38M             | 2    | ✅ files       | —                                                                                         |
...
Data is hashed, deduplicated, and compressed with **ZSTD** on the fly before being written to the datastore. Backups are taken in **snapshot mode**: the hyperviseur momentarily freezes VM/LXC state (RAM + filesystem), reads the data, then releases the snapshot. Services continue running with no downtime.
...
| Appflowy            | PostgreSQL | `/opt/k3s-data/appflowy/`            | `pg_dump -U appflowy appflowy > appflowy.sql`                                                                    | `/mnt/data/backups/dumps/appflowy.sql`                     |
...
| Scrutiny            | SQLite     | `/opt/k3s-data/scrutiny/`            | `sqlite3 scrutiny.db .dump > scrutiny.sql`                                                                       | `/mnt/data/backups/dumps/scrutiny.sql`                     |
